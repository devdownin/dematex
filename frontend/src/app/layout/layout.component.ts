    ...
    <div class="flex min-h-screen text-[#171c1f] antialiased bg-[#f6fafe]">
      ...
      <aside class="hidden md:flex flex-col h-screen w-64 bg-slate-100 py-6 px-4 space-y-2 sticky top-0 font-['Inter'] text-sm tracking-wide border-r border-slate-200">
        <div class="flex items-center gap-3 px-2 mb-8">
          ...
          <div>
            <h2 class="font-bold text-slate-900 leading-tight">{{ getScope() }}</h2>
        ...
        <nav class="flex-grow space-y-1">
          ...
          <a routerLink="/client-apis" routerLinkActive="bg-white text-slate-950 font-semibold shadow-sm" class="flex items-center gap-3 px-4 py-2.5 text-slate-600 hover:text-slate-900 hover:bg-slate-200/50 rounded-lg transition-all">
        ...
        <div class="pt-6 border-t border-slate-200 space-y-1">
          ...
          <button type="button" (click)="logout()" class="flex w-full items-center gap-3 px-4 py-2.5 text-slate-600 hover:text-slate-900 hover:bg-slate-200/50 rounded-lg transition-all">
      ...
      <main class="flex-1 flex flex-col min-w-0">
        ...
        <header class="flex justify-between items-center px-8 py-4 w-full bg-slate-50 sticky top-0 z-50 font-['Inter'] antialiased tracking-tight border-b border-slate-200">
          <div class="flex items-center gap-6">
            <h1 class="text-xl font-black tracking-tighter text-slate-900">{{ getScope() }}</h1>
...
export class LayoutComponent implements OnInit, OnDestroy {
  ...
  getScope(): string {
    const u = this.user();
    if (!u) return 'Guichet unique';
    if (u.legalEntityCode) return u.legalEntityCode;
    if (u.allowedIssuer) return u.allowedIssuer;
    return 'Super Admin';
  }
